...
mise_forme_coef_hiers = function(coefs, nstr)
{
  ...
  mise_forme_coef_hiers_R = function(coefs, nstr,ref){
    p        = length(coefs)/(nstr+1)
    Coefbar  = coefs[1:p]
    Coeforme = NULL
    coefsred = coefs[-(1:p)]
    for (c in 1:nstr)
    {
      Coeforme = cbind(Coeforme, coefsred[((c-1)*p+1):(c*p)])
    }
    Coeforme1=matrix(NA,nrow = p,ncol = (nstr+1))
    Coeforme1[,-ref]=Coeforme
    Coeforme1[,ref]=rep(0,p)
    return(c(Coefbar,as.vector(Coeforme1)))	
  }
...
CondLogist_RefLasso <- function(X,y,pairs,matchSet,method=c("BIC","BIC-R","CV","CV-OSL"),ref=1){
  ...
  if(balanced== FALSE){tauks=sqrt(KK)*sqrt(ncs/sum(ncsK))}
  }
  ...
  for (rk in ratio_lam1_lam2) {
   ...
   CVMEAN= c(CVMEAN, mod1$meancv)
    ...
    ALL_COEFS = cbind(ALL_COEFS, apply(bbeta,2,function(v){mise_forme_coef_hiers_R(Matrix(diag(c(rep(1/rk,p), rep(1,K*(p)))), sparse=T)%*%MatDiagWeightsInv%*%v,K,ref)}))
  ...
  relaxed = sel.BIC.relaxed(ALL_COEFS,X1,Y2,Z1,pairs,p,KK)
  }
  ...
  for (rk in ratio_lam1_lam2) {
    ...
    rk_mat = sparseMatrix(i = 1:(p*(K+1)), j = 1:(p*(K+1)) , x=c(rep(1/rk,p), rep(1,K*(p))) )
      ...
      WIter=1/abs(ALL_COEFS[,which.min(CVMEAN)][-((ref*p+1):((ref+1)*p))]+1e-4)
   ...
   mod0         =  clogitLasso(calX.Hier.rat,Y2,strata = pairs,nbfraction = 50,epsilon = ratio)
    ...
    ALL_COEFSIter = cbind(ALL_COEFSIter, apply(bbeta,2,function(v){mise_forme_coef_hiers_R(Matrix(diag(c(rep(1/rk,p), rep(1,K*(p)))), sparse=T)%*%MatDiagWeightsInv%*%MatDiagWeightsIterInv%*%v,K,ref)}))
  ...
  if ("BIC" %in% Criteria) {
    bic=refo(vec = ALL_COEFS[,which.min(BIC)],KK)
  }
  if ("BIC-R" %in% Criteria) {
    bicR=refo(vec = relaxed[[1]][[which.min(relaxed[[2]])]]$coef.relax,KK)
  }
  if ("CV" %in% Criteria || "CV-OSL" %in% Criteria) {
    cvmean= refo(vec = ALL_COEFS[,which.min(CVMEAN)],KK)
    ...
    eval(parse(text = paste0("cv_",cc,"se  = refo(vec =ALL_COEFS[,OneSe] ,KK)")))
    }
  }
  if ("CV-OSL" %in% Criteria) {
    cvmeaniter= refo(vec = ALL_COEFSIter[,which.min(CVMEANIter)],KK)
    ...
    eval(parse(text = paste0("cv_",cc,"se_iter  = refo(vec =ALL_COEFSIter[,OneSeiter] ,KK)")))
    }
