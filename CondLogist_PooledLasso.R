...
CondLogist_PooledLasso <- function(X,y,pairs,matchSet,method=c("BIC","BIC-R","CV","CV-OSL")){
  strata=matchSet
  NumFolds=10
  W=NULL
  Criteria=method
  ...
  if (Test==0) {
    ...
    for (k in 1) {
      ...
      if ("CV" %in% Criteria || "CV-OSL" %in% Criteria) {
        ...
        mod1         =  CV.clogit(cbind(y_k,x_k,Y1),NumFolds,mod0$fraction)
      ...
      if ("BIC" %in% Criteria) {
        ...
        bic=matrix(rep(as.vector(MatDiagWeightsInv%*%as.vector(mod0$beta[which.min(BIC),])),K),ncol=K)
      }
      if ("CV" %in% Criteria || "CV-OSL" %in% Criteria) {
        cvmean=matrix(rep(as.vector(MatDiagWeightsInv%*%as.vector(mod0$beta[which.min(mod1$meancv),])),K),ncol=K)
        ...
        for (cc in c(1,1.39,1.96)) {
          ...
          eval(parse(text = paste0("cv_",cc,"se  =matrix(rep(as.vector(MatDiagWeightsInv%*%as.vector(mod0$beta[OneSe,])),K),ncol=K)")))
        }
      }
      
      if ("BIC-R" %in% Criteria) {
        relax = sel.BIC.relaxed_IND(t(mod0$beta), x_k, y_k ,rep(1:(length(y_k)/2),each=2), p)
        bicR=matrix(rep(as.vector(MatDiagWeightsInv%*%as.vector(relax[[1]][[which.min(relax[[2]])]]$coef.relax)),K),ncol=K)
    ...
    if ("CV-OSL" %in% Criteria) {
      ...
      for (k in 1:1) {
        ...
        mod1         =  CV.clogit1(cbind(y_k,x_k%*%MatDiagWeightsInv,Y1),NumFolds,mod0$fraction)
        ...
        cvmeaniter=matrix(rep(as.vector(MatDiagWeightsIterInv%*%MatDiagWeightsInv%*%as.vector(mod0$beta[which.min(CVMEANIter),])),K),ncol=K)
        ...
        for (cc in c(1,1.39,1.96)) {
          ...
          eval(parse(text = paste0("cv_",cc,"se_iter  =matrix(rep(as.vector(MatDiagWeightsIterInv%*%MatDiagWeightsInv%*%as.vector(mod0$beta[OneSeiter,])),K),ncol=K)")))
        }
